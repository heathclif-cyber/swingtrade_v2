...
class ModelMeta(db.Model):
    ...
    __table_args__ = (
        db.UniqueConstraint("coin_id", "model_type", "run_id", name="uq_model_meta_coin_type_run"),
    )
    ...
    run_id                = db.Column(db.Text)                   # YYYYMMDD_HHMMSS
    ...
    def __repr__(self):
        return f"<ModelMeta {self.model_type} run={self.run_id} coin_id={self.coin_id}>"
